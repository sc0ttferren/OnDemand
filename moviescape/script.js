...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ScottSuite Popup Handling
    function showScottSuitePopup() {
        const popup = document.getElementById('popupSS');
    ...
    function hideScottSuitePopup() {
        const popup = document.getElementById('popupSS');
        popup.style.display = 'none';
    }

    function showScottSuite() {
        const scottSuiteContent = document.getElementById('scott-suite-content');
        scottSuiteContent.innerHTML = '<h2>ScottSuite</h2>';
        showScottSuitePopup();
    }

    function checkModeratorStatus() {
        if (userpermKey && ["46207", "46208", "46209"].includes(userpermKey)) {
            const scottSuiteContent = document.getElementById('scott-suite-content');
            scottSuiteContent.textContent = "It seems like you're a moderator, would you like to open up ScottSuite?";
            ...
            scottSuiteContent.appendChild(openScottSuiteButton);
            showScottSuitePopup();
        }
    }

    // Show the ScottSuite popup if user is a moderator
